...
public:
    ...
    for(Dataset::const_iterator c_iter=filenames.begin(); c_iter != filenames.end(); ++c_iter)
      {
		...
		CImg<double> class_vectors(30*30*3, filenames.size(), 1);
  ...
  system(" ./svm_multiclass_learn -c 2000 dat.txt");
  }
...
protected:
  ...
  CImg<double> extract_features(const string &filename)
    {
      return (CImg<double>(filename.c_str())).resize(30,30,1,3).unroll('x');
    }
